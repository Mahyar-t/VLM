...
public class PythonBridge {
    ...
    public String train(String dataDir,
        ...
        if (model != null && !model.isBlank())      cmd.addAll(List.of("--model", model));
        if (epochs != null)                           cmd.addAll(List.of("--epochs", epochs.toString()));
        if (batchSize != null)                        cmd.addAll(List.of("--batch-size", batchSize.toString()));
        if (lr != null)                               cmd.addAll(List.of("--lr", lr.toString()));
        if (device != null && !device.isBlank())     cmd.addAll(List.of("--device", device));
        if (checkpoint != null && !checkpoint.isBlank()) cmd.addAll(List.of("--checkpoint", checkpoint));
        if (classMapOut != null && !classMapOut.isBlank()) cmd.addAll(List.of("--save-class-map", classMapOut));
